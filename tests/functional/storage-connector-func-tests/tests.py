...
class StorageConnectorTest(test_utils.BaseCharmTest):
    ...
    def test_iscsi_connector_config_changed(self):
        ...
        zaza.model.wait_for_application_states(
            states={
                ...
                "ubuntu": {"workload-status": "active", "workload-status-message-regex": "^$"},
                "ubuntu-target": {
    ...
    def test_validate_iscsi_session(self):
        ...
        logging.info(
            """iscsiadm -m session: Stdout: {}, Stderr: {}, """
            """Code: {}""".format(run["Stdout"], run["Stderr"], run["Code"])
        )
        assert run["Code"] == "0"
    ...
    def test_defer_service_restarts(self):
        ...
        logging.info(
            """Service start time before config change: {}""".format(active_time_pre_check)
        )
        ...
        logging.info(
            """Service start time after config change: {}""".format(active_time_post_check)
        )
        ...
        zaza.model.run_action(
            unit.entity_id, "restart-services", action_params={"deferred-only": True}
        )
        zaza.model.wait_for_application_states(
            states={
                "storage-connector": {
        ...
        logging.info(
            """Service start time after service restart action: {}""".format(
