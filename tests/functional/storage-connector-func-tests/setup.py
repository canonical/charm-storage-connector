...
def basic_target_setup():
    ...
    setup_cmds = ["apt install --yes tgt", "systemctl start tgt", "open-port 3260"]
    for cmd in setup_cmds:
        zaza.model.run_on_unit(unit.entity_id, cmd)
...
def configure_iscsi_target():
    ...
    write_file = (
        """echo -e '<target {}>\n\tbacking-store {}\n\tinitiator-address """
        """{}\n\tincominguser {} {}\n\toutgoinguser {} {}\n</target>' """
        """ | sudo tee /etc/tgt/conf.d/iscsi.conf""".format(
            lun, backing_store, initiator_address, username, password, username_in, password_in
        )
    )
...
def configure_iscsi_connector():
    ...
    zaza.model.wait_for_application_states(
        states={
            ...
            "ubuntu-target": {"workload-status": "active", "workload-status-message-regex": "^$"},
        }
    )
